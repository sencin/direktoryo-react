...
export default function EditResource() {
  ...
  const inputStyles =
    "w-full bg-nature-nav/30 border-2 border-nature-sage/20 p-4 outline-none focus:border-nature-sage transition-colors text-sm rounded-none";

  const labelStyles =
    "text-[10px] font-black uppercase tracking-widest text-nature-sage";

  // ⏳ LOADING
  if (fetching) {
    return (
      <div className="min-h-screen bg-nature-bg flex items-center justify-center font-mono">
        <p className="text-[10px] uppercase tracking-[0.3em] animate-pulse opacity-50 text-nature-cream">
  ...
  if (notFound) {
    return (
      <div className="min-h-screen bg-nature-bg flex flex-col items-center justify-center font-mono text-nature-cream">
        <p className="text-[12px] uppercase tracking-[0.3em] opacity-60 mb-6">
          Resource Not Found
        </p>
        ...
        <div className="flex gap-4">
          <button
            onClick={() => navigate(-1)}
            className="text-[10px] uppercase tracking-[0.2em] border border-nature-sage px-6 py-3 hover:bg-nature-sage hover:text-nature-bg transition"
          >
            Go Back
          </button>
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-10">
          ...
          <button
            ...
            disabled={loading}
            className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-red-500/40 hover:text-red-500 transition-colors disabled:opacity-30"
          >
        ...
        <header className="mb-12 border-l-4 border-nature-sage pl-6">
          ...
          <p className="text-[9px] md:text-[10px] text-nature-sage font-bold uppercase tracking-widest mt-1">
            Ref: {formData.identifier || id}
          </p>
        </header>

        {/* FORM */}
        <form
          key={formData.identifier || "form"}
        ...
        >
          ...
          <button
            disabled={loading}
            className="w-full py-6 bg-nature-sage text-nature-bg font-black uppercase tracking-[0.3em] text-xs flex items-center justify-center gap-3 disabled:opacity-30"
          >
            {loading ? (
