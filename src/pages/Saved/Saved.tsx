...
const ResourceCard = memo(({ item, onToggleSave }: { item: any, onToggleSave: (id: number) => void }) => {
      ...
      <div className="flex gap-5">
        ...
        <div className="w-1/3 shrink-0">
          <div className="border-2 border-nature-sage/30 shadow-[4px_4px_0_0_rgba(0,0,0,0.1)] dark:shadow-[4px_4px_0_0_rgba(0,0,0,0.4)] aspect-[3/4] bg-nature-nav overflow-hidden">
            {item.image_url ? (
              <img src={item.image_url} alt={item.name} className="w-full h-full object-cover grayscale-[0.2] group-hover:grayscale-0 transition-all duration-300" />
            ) : (
              <div className="flex items-center justify-center h-full text-2xl font-black opacity-10">{item.name?.[0]}</div>
            )}
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0 relative">
          ...
          >
            <Bookmark size={22} className={isSaved ? "fill-nature-sage text-nature-sage" : ""} />
          </button>
      ...
      <main className="p-6 pb-24">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-24 opacity-20 animate-pulse">
            ...
            <span className="text-xs font-black uppercase tracking-[0.3em]">Synchronizing...</span>
          </div>
